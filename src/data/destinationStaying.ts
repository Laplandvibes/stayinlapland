...
const levi: Record<string, StayingNote> = {
  ...
  slopeside: {
    ...
    body: {
      ...
      ko: '스키 인/아웃은 당연하게 여기지 말고 확인할 대목입니다. 레비툰투리는 여러 사면에 슬로프가 있어, 리프트로 이어지는 샬레라도 머릿속에 그린 그 리프트가 아닐 수 있습니다. 어느 리프트인지, 하루가 끝날 무렵 돌아오는 길은 어떤지 물어보세요.',
  ...
  beyond: {
    ...
    body: {
      ...
      de: 'Hütten weiter draußen tauschen Gehweite gegen dunklen Himmel und Ruhe. Ein Auto ist nötig, und bei Schneefall sind die letzten Kilometer die langsamen – dafür muss das Polarlicht nicht gegen Straßenlaternen ankommen.',
...
const inari: Record<string, StayingNote> = {
  ...
  shore: {
    ...
    body: {
      ...
      ko: '캐빈은 호숫가를 따라, 그리고 숲 안쪽에 서 있습니다. 공간은 넉넉하고 그 사이에는 거의 아무것도 없습니다. 나가기 전에 드실 것을 사 두세요. 상점은 마을에 있지 길가에 있지 않습니다.',
...
const yllas: Record<string, StayingNote> = {
  ...
  liftsOrTracks: {
    ...
    body: {
      ...
      ko: '슬로프는 산 양쪽으로 내려오고, 그 둘레의 숲에는 약 300킬로미터의 코스가 이어집니다. 어느 쪽을 더 많이 쓸지 정한 다음에 주소를 고르세요. 둘 사이에는 산 자체가 놓여 있습니다.',
