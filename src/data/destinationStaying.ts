...
export const stayingUi: Record<string, L12> = {
  ...
  note: {
    ...
    es: 'Zonas, no alojamientos. La parte del destino en la que duermes marca el viaje tanto como la habitación, y nada de esta sección está patrocinado.',
...
const rovaniemi: Record<string, StayingNote> = {
  ...
  river: {
    ...
    body: {
      ...
      it: 'Ounasvaara si alza sulla riva opposta, di fronte al centro. Da quella parte piste e anelli illuminati sono davanti alla porta e la città è a un ponte di distanza: sensato se sciate ogni giorno, meno se volete andare a cena a piedi.',
  ...
  glass: {
    ...
    body: {
      ...
      sv: 'Glasiglon och norrskensstugor är en rumstyp, inte ett område. De ligger utanför staden just för att himlen är mörk där, så lös transporten innan du bokar: för de flesta betyder det bil eller en beställd transfer.',
      ...
      es: 'Los iglús de cristal y las cabañas de auroras son un tipo de habitación, no un barrio. Están fuera de la ciudad porque allí el cielo sigue oscuro, así que resuelve los traslados antes de reservar: en la mayoría implica coche o un transfer concertado.',
      ...
      it: 'Igloo di vetro e cabin per l’aurora sono un tipo di camera, non un quartiere. Stanno fuori città perché è lì che il cielo resta buio, quindi risolvete gli spostamenti prima di prenotare: per la maggior parte significa auto o un transfer concordato.',
      nl: 'Glazen iglo’s en poollichthutten zijn een kamertype, geen wijk. Ze liggen buiten de stad omdat de hemel daar donker blijft, dus regel het vervoer vóór u boekt: voor de meeste betekent dat een auto of een afgesproken transfer.',
      ja: 'ガラスイグルーやオーロラキャビンは部屋の種類であって、地区の名前ではありません。空が暗いままだから郊外にあるので、予約の前に行き帰りの手段を決めておくこと。多くは車か、手配した送迎が前提です。',
      ko: '글라스 이글루와 오로라 캐빈은 객실 유형이지 동네 이름이 아닙니다. 하늘이 어두운 곳이라 시 외곽에 있으니, 예약 전에 오가는 방법부터 정하세요. 대부분은 차 또는 미리 잡아둔 픽업이 전제입니다.',
      'zh-CN': '玻璃穹顶小屋和极光小木屋是房型，不是某个街区。它们建在城外，正因为那里的天空够黑，所以订房前先把往返方式定下来：多数情况意味着自驾或预约接送。',
...
const levi: Record<string, StayingNote> = {
  ...
  slopeside: {
    ...
    body: {
      ...
      es: 'Lo de «a pie de pista» conviene comprobarlo, no darlo por hecho. Levitunturi tiene laderas en más de un lado, y un chalé que llega a un remonte no siempre llega al que tenías en mente. Pregunta a cuál, y cómo es la vuelta al final del día.',
      ...
      it: 'Lo ski-in ski-out va verificato, non dato per scontato. Levitunturi ha piste su più versanti, e una baita che arriva a un impianto non arriva sempre a quello che avevate in mente. Chiedete quale, e com’è il rientro a fine giornata.',
      ...
      ko: '스키인·스키아웃은 당연하게 여기지 말고 확인할 대목입니다. 레비툰투리는 여러 사면에 슬로프가 있어, 리프트로 이어지는 샬레라도 머릿속에 그린 그 리프트가 아닐 수 있습니다. 어느 리프트인지, 하루가 끝날 무렵 돌아오는 길은 어떤지 물어보세요.',
  ...
  beyond: {
    ...
    body: {
      ...
      nl: 'Hutten verderop ruilen loopafstand in voor een donkere hemel en stilte. U hebt een auto nodig, en bij sneeuwval zijn de laatste kilometers de trage, maar het noorderlicht hoeft niet op te boksen tegen straatverlichting.',
...
const saariselka: Record<string, StayingNote> = {
  village: {
    ...
    body: {
      ...
      es: 'La mayoría de las plazas están en el pueblo al pie de la montaña, con los remontes y los restaurantes a pie. Es la base más sencilla si prefieres no conducir, y el camino de vuelta más corto en la oscuridad.',
      fr: 'L’essentiel des lits se trouve au village, au pied du mont, remontées et restaurants accessibles à pied. C’est la base la plus simple si vous préférez ne pas conduire, et le plus court retour dans le noir.',
      it: 'La maggior parte dei letti è nel paese ai piedi della montagna, con impianti e ristoranti raggiungibili a piedi. È la base più semplice se preferite non guidare, e il rientro più breve al buio.',
      nl: 'De meeste bedden staan in het dorp aan de voet van de fjeld, met liften en restaurants op loopafstand. Het is de eenvoudigste uitvalsbasis als u liever niet rijdt, en de kortste weg terug in het donker.',
  ...
  glass: {
    ...
    body: {
      ...
      es: 'Las habitaciones con techo de cristal están repartidas por la zona, no reunidas en un punto. Comprueba la distancia a los remontes y a las tiendas antes de reservar: tan al norte, en diciembre, ese paseo es una decisión y no un detalle.',
      ...
      it: 'Le camere con tetto di vetro sono sparse nella zona, non riunite in un punto solo. Controllate la distanza dagli impianti e dai negozi prima di prenotare: così a nord, a dicembre, quella camminata è una decisione e non un dettaglio.',
      ...
      ko: '유리 지붕 객실은 한곳에 모여 있지 않고 지역 곳곳에 흩어져 있습니다. 예약 전에 리프트와 상점까지의 거리를 확인하세요. 이만큼 북쪽의 12월에 그 걷는 거리는 사소한 항목이 아니라 결정 사항입니다.',
...
const inari: Record<string, StayingNote> = {
  village: {
    ...
    body: {
      ...
      fr: 'Le village borde le lac et abrite Siida, le musée national de la culture sâme. Il est petit, et c’est précisément la raison d’y dormir : l’eau, le silence et la courte lumière d’hiver plutôt qu’un choix d’adresses.',
      ...
      nl: 'Het dorp ligt aan het meer en herbergt Siida, het nationale museum van de Sámi-cultuur. Het is klein, en juist dat is de reden om er te blijven: het water, de stilte en het korte winterlicht in plaats van een keuze aan adressen.',
  ...
  ivalo: {
    ...
    body: {
      ...
      es: 'El aeropuerto está en Ivalo, y allí vive también la mayoría de los habitantes del municipio; el pueblo de Inari es el más pequeño de los dos. Alojarse en Ivalo acorta el primer y el último día del viaje; alojarse en Inari te deja junto al lago todos los días intermedios.',
      ...
      it: 'L’aeroporto è a Ivalo, dove vive anche la maggior parte degli abitanti del comune; il villaggio di Inari è il più piccolo dei due. Una camera a Ivalo accorcia il primo e l’ultimo giorno del viaggio; una camera a Inari vi mette in riva al lago per tutti i giorni in mezzo.',
  ...
  shore: {
    ...
    body: {
      ...
      it: 'Le baite stanno lungo la riva e dentro il bosco, con molto spazio e pochissimo in mezzo. Fate la spesa prima di uscire: i negozi sono nei villaggi, non lungo la strada.',
      ...
      ko: '캐빈은 호숫가를 따라, 그리고 숲 안쪽에 서 있습니다. 공간은 넉넉하고 그 사이에는 거의 아무것도 없습니다. 나가기 전에 먹을 것을 사두세요. 상점은 마을에 있지 길가에 있지 않습니다.',
...
const yllas: Record<string, StayingNote> = {
  villages: {
    ...
    body: {
      ...
      fr: 'Un mont, deux villages, et le choix compte davantage que la carte ne le laisse croire, car ils sont sur des versants opposés. Des navettes ski relient les deux aux remontées : aucun n’est isolé, mais passer de l’un à l’autre est un trajet, pas une promenade.',
      ...
      nl: 'Eén fjeld, twee dorpen, en de keuze telt zwaarder dan de kaart doet vermoeden, want ze liggen aan weerszijden. Skibussen verbinden beide met de liften, dus geen van beide is afgesloten, maar van het ene naar het andere is een rit en geen wandeling.',
  ...
  liftsOrTracks: {
    ...
    body: {
      ...
      es: 'Las pistas bajan por las dos laderas de la montaña y unos trescientos kilómetros de circuitos recorren el bosque que la rodea. Decide cuál de las dos cosas usarás más antes de elegir dirección, porque en medio está la propia montaña.',
      fr: 'Les pistes descendent des deux versants du mont et environ trois cents kilomètres de tracés courent dans la forêt alentour. Décidez de ce que vous utiliserez le plus avant de choisir une adresse, car entre les deux se dresse le mont lui-même.',
      it: 'Le piste scendono su entrambi i versanti della montagna e attorno, nel bosco, corrono circa trecento chilometri di anelli. Decidete quale delle due userete di più prima di scegliere l’indirizzo, perché in mezzo c’è la montagna stessa.',
      nl: 'Pistes lopen langs beide zijden van de fjeld naar beneden en rondom slingert zo’n driehonderd kilometer loipe door het bos. Bepaal wat u het meest gaat gebruiken vóór u een adres kiest, want daartussen ligt de fjeld zelf.',
      ja: 'ゲレンデは山の両側を落ち、まわりの森にはおよそ三百キロのコースがめぐっています。どちらをより多く使うのかを決めてから住所を選んでください。ふたつのあいだには山そのものがあります。',
      ko: '슬로프는 산 양쪽으로 내려오고, 그 둘레의 숲에는 약 삼백 킬로미터의 코스가 이어집니다. 어느 쪽을 더 많이 쓸지 정한 다음에 주소를 고르세요. 둘 사이에는 산 자체가 놓여 있습니다.',
